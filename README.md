...
| 地址 | 谁用 |
|---|---|
| `http://43.142.33.45:8443/?k=<观看令牌>` | 观众链接，直接发人 |
| `http://43.142.33.45:8443/whip/share01?k=<推流令牌>` | OBS 推流地址（服务选 WHIP） |

真实令牌在 `.env` 里（`.env` 不入库）。`deploy/deploy.ps1` 跑完会把两条完整链接打印出来。
...
轮换令牌：改 `.env` 里对应那一项，重跑 `deploy.ps1 -SkipBuild`。
改房间号或端口同理（房间号同时写在 nginx 的 location、MediaMTX 的 paths 和前端构建里，
...
**实测结果（2026-09-19，23 条断言全过）**：

| 项 | 实测 |
|---|---|
| WHIP 推流建立 | PeerConnection connected |
| 观众页出画面（WHEP） | **2.3 秒**，1280×720，有声音 |
| 拦掉 WHEP 后自动降级 HLS | **3.0 秒**出画，随后持续推进（3 秒推进 3.49 秒） |
| 服务端转码 | 无：推流端 1280×720 H264+Opus → 收看端一致 |
| WHEP 会话 DELETE | HTTP 200（令牌没被 nginx 拼两遍） |
| 越权 | 错令牌 401、错房间 404 |
