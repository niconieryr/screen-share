...
| 地址 | 谁用 |
|---|---|
| `https://share.polarbear.net.cn/screen` | **观看链接**，打开就出画，直接发人 |
| `https://share.polarbear.net.cn/whip/share01?k=<推流令牌>` | OBS 推流地址（服务选 WHIP，这条**带令牌**） |

`https://share.polarbear.net.cn/`（不带路径）会 302 跳到 `/screen`，别的路径一律 404
（所以路径本身能挡住随手扫描的人）。推流令牌在 `.env` 里（`.env` 不入库），
`deploy/deploy.ps1` 跑完会把两条链接打印出来。
...
| 位置 | 端口 | 用途 | 谁放行 |
|---|---|---|---|
| 公网 | `443/tcp` | 唯一入口：页面 + WHIP/WHEP/HLS | 面板 nginx（既有站点入口，不用为本项目新开端口） |
| 公网 | `8189/udp` | WebRTC 媒体（DTLS-SRTP 加密） | **腾讯云安全组 + 本机 ufw** |
| 回环 | `8443` | edge：静态站 + 令牌校验 + 反代 MediaMTX | 只绑 `127.0.0.1`，公网进不来 |
...
所以 WebRTC 媒体口只能用 8189）；面板那条 443 上只是多了一个 `share.polarbear.net.cn` 站点。

> 2026-09-22 之前本项目自己对外开 `8443/tcp`（明文 http + IP 端口）。现在 edge 只绑回环，
> 那个端口不再监听，**腾讯云安全组里那条 `8443/tcp` 规则可以删掉了**。
> 真要临时恢复 IP 直连：`.env` 的 `EDGE_BIND` 改成 `0.0.0.0` 重跑部署（规则还在，改完即用）。
...
```
OBS 32 ──WHIP(https POST)──▶ 面板 nginx:443 ──▶ edge:8443 ──▶ 127.0.0.1:8889 MediaMTX
    └── 媒体 DTLS-SRTP / UDP 8189（用 IP，不用域名）──────────────┘
                                                          │
观众浏览器 ◀── WHEP(https) ── 面板 nginx:443 ──▶ edge:8443 ─┤ 主路径，<1s，Opus 直通
观众浏览器 ◀── LL-HLS(https) ─ 面板 nginx:443 ──▶ edge:8443 ─┘ 兜底，2-4s，UDP 被封时自动切
```
...
2. **OBS 静默拒绝自签证书的 WHIP**（不弹「证书不受信任」，就是连不上）。所以当年没有域名时
   只能明文 http + IP；现在有真证书了就走 https 域名 —— 这条坑变成「别拿自签证书糊弄 OBS」。
...
2026-09-22 之前这个项目跑在「明文 http + IP:8443」上，那是**没有域名时的妥协**：
签不了真证书 → 只能明文 → 浏览器报「不安全」、`navigator.clipboard` 用不了、
非标端口还可能被运营商降优先级。域名到手之后这些一次全解决：

| 当年的妥协 | 现在 |
|---|---|
| `http://43.142.33.45:8443/screen`，链接难念 | `https://share.polarbear.net.cn/screen` |
| 地址栏「不安全」 | 泛域名真证书（面板 ACME 自动续签，每日按 ARI 铺到全部站点） |
| 复制链接要靠 textarea + `execCommand` 兜底 | 安全上下文里 `navigator.clipboard` 可用（前端本来就有两条路，自动挑好的那条） |
| 8443 是非标端口，可能被限速/降优先级 | 443，谁都不会动它 |
| 明文 http 会被腾讯云按未备案域名拦 | 域名走 TLS，拦不到（它拦的是明文请求里的 Host） |

**结构没变**：还是「面板 nginx 终止 TLS → edge 反代 → MediaMTX」，只是 edge 从
「对外监听 8443」退回「只绑回环」。这也是为什么这次切换不需要改前端 ——
页面里的地址全部由 `window.location.origin` 拼出来，换入口它自己就跟着换。

**观看链接本身仍然不保密**（`/screen` 打开就能看）。想加锁就把 `VIEW_TOKEN` 填上，
...
- **`8443` 已经不对公网监听**（`EDGE_BIND` 默认 `127.0.0.1`）。要按 IP 直连得先把它改回
  `0.0.0.0` 重跑部署 —— 那条路是应急用的，日常入口只有 https 域名这一条。
